...
// Convert input image to grayscale (Vectorized 4-pixel access)
                                    ...
                                    int width, int height, float threshold_sq)
{
    ...
    // Apply 3x3 Sobel filter directly (texture cache handles spatial locality)
    for (int ky = -1; ky <= 1; ky++)
    {
        for (int kx = -1; kx <= 1; kx++)
        {
            float px = tex2D<unsigned char>(tex, gx + kx, gy + ky);
            Gx += px * c_sobelX[k_sobel];
            Gy += px * c_sobelY[k_sobel];
            k_sobel++;
        }
    }

    // Compute squared gradient magnitude to avoid expensive sqrtf
    float mag_sq = Gx * Gx + Gy * Gy;

    // Apply threshold and ROI mask
    filtered[idx] = (mag_sq > threshold_sq && mask[idx] > 0) ? 255 : 0;
}
...
static cudaTextureObject_t create_texture(unsigned char *d_ptr, int width, int height)
{
...
int main()
{
    ...
    if (!cap.isOpened())
    {
        std::cerr << "Error opening input.mp4. Make sure the file exists in the same directory.\n";
        return -1;
    }
    ...
    CUDA_CHECK(cudaMemcpy(d_mask, h_mask.data, width * height, cudaMemcpyHostToDevice));
    ...
    // Fixed Grid Math for vectorized load
    dim3 grid_gray(((width + VEC - 1) / VEC + 31) / 32, (height + 7) / 8);

    dim3 block_std(TILE_W, TILE_H);
    dim3 grid_std((width + TILE_W - 1) / TILE_W, (height + TILE_H - 1) / TILE_H);

    dim3 block_hough(32, 8);
    dim3 grid_hough((width + 31) / 32, (height + 7) / 8);
    ...
    while (true)
    {
        ...
        // Ensure OpenCV Mat is continuous before raw memory copy
        ...
        CUDA_CHECK(cudaMemcpyAsync(h_accumulator, d_accumulator_gpu[buf],
                                   acc_size, cudaMemcpyDeviceToHost, stream[buf]));
        ...
        for (int t = 0; t < num_theta; t++)
        {
            ...
            if (t > 70 && t < 110) continue; // Ignore horizontal lines
            ...
            for (int r = 0; r < num_rho; r++)
            {
                ...
                else
                {
                    if (v > right_max)
                    {
                        right_max = v; right_t = t; right_r = r;
                    }
                }
            }
        }
    ...
    cap.release();
    out.release();
    std::cout << "Processing Complete. Output saved to output.mp4\n";
    return 0;
}